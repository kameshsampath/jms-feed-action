...
public class JmsFeedAction {

  public static JsonObject main(JsonObject request) {
    JsonObject response = new JsonObject();
    //TODO validation
    try {
      JmsProvider provider = Feign.builder()
        .client(new OkHttpClient())
        .decoder(new GsonDecoder())
        .encoder(new GsonEncoder())
        .logger(new Slf4jLogger(JmsProvider.class))
        .target(JmsProvider.class, "http://jms-feed-provider.myproject:8080");
